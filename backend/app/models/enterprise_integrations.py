...
class N8nConnection(Base):
    """An organization's n8n instance connection details. One per organization."""
    ...
    connection_token: Mapped[str] = mapped_column(String(64), nullable=False, unique=True, index=True, default=generate_uuid)
...
class N8nWorkflowMapping(Base):
    """Maps an internal trigger event (e.g. 'lead.created') to an n8n webhook URL, plus an
    optional structured field-mapping contract a future visual-mapping UI would edit."""
    __tablename__ = "n8n_workflow_mappings"

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=generate_uuid)
    organization_id: Mapped[str] = mapped_column(String(36), ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False)
    connection_id: Mapped[str] = mapped_column(String(36), ForeignKey("n8n_connections.id", ondelete="CASCADE"), nullable=False)
    name: Mapped[str] = mapped_column(String(150), nullable=False)
    trigger_event: Mapped[str] = mapped_column(String(60), nullable=False, index=True)  # e.g. lead.created, whatsapp.message.received
    ...
    field_mapping_json: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    status: Mapped[str] = mapped_column(String(20), default="active")  # active, paused
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)


class N8nExecutionLog(Base):
    """One row per trigger attempt (or inbound callback), including retry bookkeeping."""
    __tablename__ = "n8n_execution_logs"

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=generate_uuid)
    organization_id: Mapped[str] = mapped_column(String(36), ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False)
    mapping_id: Mapped[Optional[str]] = mapped_column(String(36), ForeignKey("n8n_workflow_mappings.id", ondelete="SET NULL"), nullable=True)
    direction: Mapped[str] = mapped_column(String(10), nullable=False)  # trigger, callback
